...
int main(int argc, char** args) //int argc, char** args
{
	...
	n = stoi(num_of_vertices); // string-to-integer

	cin >> num_of_edges; // Take m number of edges
	m = stoi(num_of_edges); // string-to-integer

	//cout << n << " " << m << endl;

	int size = n + 1;
	// Using n+1 as the adjacency list size. I want the index to match the node # so I won't be using index 0 because node 0 doesn't exist
	node_entry** vertices = new node_entry * [size];
	min_heap* array = new min_heap[m];

	for (int i = 0; i <= n; i++) // initializing every node in adjacency list
	{
	...
	for (int i = 0; i < m; i++) // initializing every node in the array (which I don't end up using for the min_heap)
	{
	...
	get_edges(vertices, m); // Call to get_edges -> graph.cpp
	create_array(vertices, array, m, n); // Call to create an array of all edges, don't end up using this anyway later on
	...
	cin >> line; // Grab the first word of the instruction
			...
			if (flag == 1)
			{
				...
				*/
				...
				{					
					...
					{
						/*min_node* S = dijkstra(vertices, s, t, n);
						...
						for (int i = 0; i <= n; i++)
						{
							...
							{
								cout << S[i].current_node->node_position << endl;
							}							
						}*/
